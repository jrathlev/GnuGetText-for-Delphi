...
type
  ...
  public
    { Public declarations }
    function Execute (const APos : TPoint; Titel,Desc : string;
                      HList : TStrings; ADelete,AEdit : boolean;
                      var AIndex : integer) : boolean;
  end;

function EditHistList(const APos : TPoint; Titel,Desc : string;
                      HList : TStrings; ADelete,AEdit : boolean;
...
uses System.Math, GnuGetText, ExtSysUtils, InpText, WinUtils, ShowMessageDlg, ImageLoader;
...
begin
  ...
  MinWidth:=lbxStringList.Width; MaxWidth:=Monitor.Width div 3;
  end;

procedure TEditHistListDialog.FormAfterMonitorDpiChanged(Sender: TObject;
...
begin
  Clientwidth:=ClientWidth-lbxStringList.Width+2*lbxStringList.Left+ListWidth;
  ...
  with lbxStringList do if ItemIndex>=0 then begin
    ...
    if InputText(BottomRightPos(btnEdit),dgettext('dialogs-svg','Edit item'),lbDesc.Caption,false,'',nil,false,s) then begin
      ...
      if (n>=0) and (n<>ItemIndex) then
        ErrorDialog(TopLeftPos(btnEdit),TryFormat(dgettext('dialogs-svg','Item "%s" already exists!'),[s]))
      else begin
        Items[ItemIndex]:=s;
...
{------------------------------------------------------------------- }
function TEditHistListDialog.Execute (const APos : TPoint; Titel,Desc : string;
                    HList : TStrings; ADelete,AEdit : boolean;
                    var AIndex : integer) : boolean;
var
  i : integer;
...
begin
  ...
  btnEdit.Visible:=AEDit; ListWidth:=0;
  with lbxStringList do Canvas.Font:=Font;
  with HList do for i:=0 to Count-1 do begin
    lbxStringList.AddItem(Strings[i],Objects[i]);
    ListWidth:=Max(ListWidth,lbxStringList.Canvas.TextWidth(Strings[i]));
    end;
  if ListWidth<MinWidth then ListWidth:=MinWidth;
  if ListWidth>MaxWidth then ListWidth:=MaxWidth;
