...
// The required SVG functions in Direct2D are supported as of
// Windows 10 1703 - Creators Update (Build 15063)
// see: https://learn.microsoft.com/en-us/windows/win32/direct2d/svg-support
...
// Invert color, keep the alpha channel untouched   - JR May 2025
function InvertColor(Color : TD2D1ColorF) : TD2D1ColorF;

  function SingleXor(val : single) : single;
  begin
    Result:=(round(255*val) xor $FF)/255;
    end;

begin
  Result:=D2D1ColorF(SingleXor(Color.r),SingleXor(Color.g),SingleXor(Color.b),Color.a);
end;
...
procedure InvertSubtree(const Element: ID2D1SvgElement);
begin
  TransformSvgElement(Element,
    procedure(const Element: ID2D1SvgElement)
    Var
      OldColor: TD2D1ColorF;
      NewColor: TD2D1ColorF;
      procedure InvertAttribute(Attr: PWideChar);
      Var
        IsInherited: Bool;
      begin
        if Element.IsAttributeSpecified(Attr, @IsInherited) and not IsInherited then
    ...
    begin
      InvertAttribute('fill');
      InvertAttribute('stroke');
      InvertAttribute('stop-color');
    end);
end;
...
function TD2DSVG.GetInvert: Boolean;
begin
  Result := FInvert;
end;
...
procedure TD2DSVG.SetInvert(const IsInverted : Boolean);
Var
  Root: ID2D1SvgElement;
begin
  if IsInverted<>FInvert then begin
    FInvert:=IsInverted;
    if fInvert and Assigned(fSvgDoc) then begin
      fSvgDoc.GetRoot(Root);
      InvertSubtree(Root);
    end;
    end;
...
// Support functions requires at least Windows 10, Build 15063 (Creators Update)
function WinSvgSupported: Boolean;
begin
